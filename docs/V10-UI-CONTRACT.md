...
V10 지도는 별도 제품처럼 새 UI 키트를 만드는 화면이 아니다. **기존 HOMEBOUND의 경기장·전광판·픽셀 아트 디자인 언어를 이어받는다.**

- 지도 화면은 기존 `stadium.png` 경기장 세계 안에 있어야 한다.
- 지도 노드를 일반적인 둥근 네모 카드/대시보드 타일로 만들지 않는다.
- 각 노드는 **작은 픽셀 장소 디오라마 + 야구 엠블럼 + 짧은 이름표**가 기본 형태다. 단순 아이콘만 띄우는 것도 완료로 보지 않는다.
- battle / elite / training / locker / shop / rest / boss는 서로 다른 픽셀 장소 장면을 가져야 한다. 예: 배팅 케이지, 더그아웃, 장비실, 라커룸, 일반 구장, 라이벌 구장, 보스 전광판.
- 연결선은 노드를 중심으로 관통하는 직선이 아니라, 노드 밖에서 시작/종료하는 **곡선 원정 경로**다.
- 현재 선택 가능한 경로만 HOMEBOUND의 금빛 포인트 컬러와 글로우로 강조한다. 확정 시에는 약 0.7초 동안 경로가 그려지고 목적지가 강조되는 '원정 출발' 비트를 거친다.
- 현재 위치는 기존 경기 UI의 민트/초록 계열 상태색을 사용한다.
- 강적은 주황/적갈, 보스는 금색 계열로 즉시 구분한다.
- 전투 노드에는 상대의 실제 3×3 주요 승부 존을 미니 지문으로 보여줘야 한다. 지도만 봐도 낮은 싱커형/높은 공형 등 9ZONE 고유 전략 정보가 읽혀야 한다.
- 상세 정보는 별도의 카드 여러 장으로 쪼개지 않고 **전광판/스카우팅 리포트 한 패널**에 상대·보상·위험·이후 경로를 계층화한다.
- 모바일에서 한 행 4노드는 유지하되, 텍스트 박스를 축소하는 방식이 아니라 엠블럼을 중심으로 정보량을 줄인다.
- 새 지도 스타일이 기존 `.stadium`, 전광판 색, 픽셀 아이콘과 시각적으로 단절되면 완료로 보지 않는다.

테스트는 최소한 `.v10-node-scene`, `.v10-diorama-art`, `.v10-zone-fingerprint`, cubic Bezier 경로(`C`)와 활성 `.v10-edge-live`를 확인해 이 형태가 다시 네모 타일/단순 아이콘으로 퇴행하지 않게 한다.
