...
- 각 노드는 **야구 다이아몬드형 엠블럼 + 픽셀 아이콘 + 짧은 이름표**가 기본 형태다.
- battle / elite / training / locker / shop / rest / boss는 서로 다른 픽셀 아이콘을 가져야 한다.
- 연결선은 노드를 중심으로 관통하는 직선이 아니라, 노드 밖에서 시작/종료하는 **곡선 원정 경로**다.
- 현재 선택 가능한 경로만 HOMEBOUND의 금빛 포인트 컬러와 글로우로 강조한다.
...
테스트는 최소한 `.v10-node-core`, `.v10-map-icon`, cubic Bezier 경로(`C`)와 활성 `.v10-edge-live`를 확인해 이 형태가 다시 네모 타일로 퇴행하지 않게 한다.
