# 9ZONE SHOWDOWN — README 아카이브

> 현재 안내는 [README.md](../../README.md)를 우선한다.

## 과거 로컬: v6 · 플레이 방식이 바뀌는 런 성장 (2026-09-11)
...
QA 체크: 첫 승리 → 성장 선택 → 연결 카드 선택 → 확정 한 번 → 다음 경기 → 성장 충전/사용/취소 → 결과·다음 타자 → 새로고침 복원. 성장 상세 규칙과 알려진 한계는 [인수인계](../../HANDOFF.md), [현행 규칙](../DUEL.md), [검증 기록](../STATUS.md)을 읽는다.
...
- 최신 설계·제약·QA: [docs/DUEL.md](../DUEL.md), [docs/STATUS.md](../STATUS.md).
...
[로컬 플레이](http://127.0.0.1:5176/) · [현재 규칙과 검사](../DUEL.md). 이전 저장 키는 보존. 이번 변경은 아직 공개 사이트에 재배포하지 않았다. 아래 v2/v1 설명은 역사 기록이다.
...
[현재 로컬 플레이](http://127.0.0.1:5176/) · [최신 설계/검증/한계](../DUEL.md). 기본 실행은 `npm run dev`, 검사는 `npm test`, `npm run build`. 전체 57개 테스트 통과. 같은 src/duel에 통합했으며 기존 저장은 보존했습니다.
...
현행 진입점은 `src/duel/App.jsx`. 최신 설계·검증·다음 작업은 **[docs/DUEL.md](../DUEL.md)**. 아래 LAST LIGHT/NIGHT RUN 절은 역사 자료다.
...
현행 소스: `src/reboot/App.jsx`, `engine.js`, `data.js`, `policy.js`, `audio.js`, `reboot.css`. 새 배경은 `assets/reboot/last-light-stadium.png`. 최신 설계·인수인계·검증 근거는 **[docs/REBOOT.md](../REBOOT.md)**를 먼저 읽는다.
