...
## 최우선 인수인계 비주얼 리부트 · 2026-09-18

브랜치 `visual-loop-batter`, 기준은 `origin/main`의 `f430c56`(Golden Master 03)이다.
커밋 5개를 남겼고 **push·배포하지 않았다.** `loop/PROMPT.md`가 푸시를 금지한다.

`VISUAL-REBOOT-LOOP-ENGINEERING.md` §7의 비주얼 QA 6문항을 기준으로 돌린 다섯 바퀴다.

- `f7ffcac` 액터 발밑 지면. 타석 흙·초크선·홈플레이트와 투수 마운드를 픽셀 SVG로
  직접 찍어 `assets/gm-ground/`에 넣고, 액터 컨테이너 안에 %로 배치해 세 뷰포트의
  clamp 크기를 따라가게 했다.
- `1d4bd2d` 9존 보드를 판에서 떠 있는 HUD로. 배경 .75 → .42, 셀 .91 → .72.
  카드를 놓는 중에는 .88로 또렷해지고 INFO 읽기 상태 .96은 그대로 둔다.
- `4803287` 지면 조명. 판정 타임라인을 따라 빛이 마운드와 타석 사이를 오간다.
- `ebd7cf8` 베이스 표시를 정보 위계로 갈랐다. 빈 베이스는 필드로 가라앉히고
  주자가 선 베이스는 더 또렷하게. 선택자를 `.golden-master-stage` 안으로 한정했다.
- `ca305e0` 컨택트 플래시를 즉시 점등으로 고쳤다.

다음 세션이 알아야 할 실측값:

- 스프라이트 시트는 512×512이고 알파 접지선이 **y=493**이다. v1 포즈 6종과 v2 프레임
  36장이 모두 같은 접지선을 쓴다. 발밑에 무엇을 놓든 이 값이 기준점이다.
- 판정 단계 지속은 1280×900에서 rAF로 재보면 windup 8프레임 · **impact 1프레임** ·
  slowmo 9 · release 13 · settle 8이다. impact에 전환을 걸면 밝아지기 전에 끝난다.
  `gm-contact-halo`도 impact에서 `.045s steps(2,end)`를 쓰고 있어 같은 문제를 겪을
  수 있다. 확인하지 않았다.
- 전투 진입은 MAIN RUN 버튼 → `[data-testid="v10-node-a1-entry"]` →
  `[data-testid="v10-map-cta"]` 순서다. 노드 클릭만으로는 들어가지 않는다.
- 이 환경에는 node가 시스템 PATH에 없다. 루프 런타임
  `~/.cache/codex-runtimes/.../node/bin/node.exe`를 쓴다. playwright와 npm/pnpm이
  없어 `scripts/v10-ui-shot.mjs`는 실행하지 못했다.

하지 않은 것과 그 이유:

- 타자/투수 픽셀 아트 재작업. 512px 원본이 PC에서 배율 1.007로 거의 1:1 렌더되고
  실루엣과 명암이 그 크기에서 버틴다. 손으로 다시 그리면 품질이 내려간다고 판단했다.
- 모바일 가로 레이아웃. §5의 실패 조건을 실측으로 대조했으나 세로·가로 스크롤 0,
  카드 6장 모두 잘림 없음이라 조건에 걸리지 않았다.
- 계획 §8의 Before/After 이미지 산출물. 화면은 세 뷰포트에서 직접 봤지만 저장소에
  남길 이미지 파일은 만들지 못했다.
- 카드 배치 중 셀 배경을 올리는 규칙 하나는 넣었다가 뺐다. 400ms를 유지하며 재도
  계산값이 움직이지 않았고 왜 지는지 끝까지 짚지 못해, 동작을 입증할 수 없는 CSS는
  남기지 않았다.

이 문서와 `docs/STATUS.md`의 최상단 절은 V9.2 기준이라 v10 현황과 어긋나 있다.
내가 만든 차이가 아니고 이번 범위 밖이라 손대지 않았다.
