...
## 새 피드백 · 2026-09-22 — 텍스트를 그래픽으로 치환

실플레이 테스터 피드백: **연출·게임감은 개선됐고 이해도가 막힌다.**
1스테이지 패배, "너무 복잡 어려움", "야구 초보나 겜초보에겐 좀더 직관적이어야 할듯".

이어진 사용자 방향 지시: **텍스트를 최대한 줄이고 그래픽·효과로 치환한다. 게임 방식은 그대로.**

기획안: [`docs/V12-UI-TEXT-TO-VISUAL.md`](./docs/V12-UI-TEXT-TO-VISUAL.md).
핵심은 흩어진 HP 피해 효율 표기 3곳(BattleReadout / 존 보드 가이드 / StackBoard)을
**투수 HP 바 위의 예상 피해 고스트 하나**로 합치는 것이다. 스윙 화면 텍스트 요소 약 113 → 28 이하가 목표다.

**검수·수정은 아스트라가 한다. 승인 전 구현 금지.**
이 항목은 아래 VISUAL RESET 기준을 취소하지 않는다. 순서는 사용자 결정 사항이며 아직 결정되지 않았다.
