...
## 새 피드백 · 2026-09-22 — 전투 화면 UI/UX 마스터 규격

테스터 피드백: 연출·게임감은 개선됐고 **이해도가 막힌다.** 1스테이지 패배, "너무 복잡 어려움".
사용자 방향: **텍스트 줄이고 그래픽·효과로 치환. 게임 방식은 그대로.**

규격: [`docs/V12-UI-MASTER-SPEC.md`](./docs/V12-UI-MASTER-SPEC.md). 검수·수정은 아스트라. **승인 전 구현 금지.**

감사에서 나온 실제 원인(재현 명령은 규격 부록 B):

- `src/duel/*.css` 21파일 5258줄, `!important` **2266개** — 전체 줄의 19.5%.
  `sts-battleboard.css`는 543줄 중 370개(68%). 새 파일이 앞 파일을 덮으며 쌓인 구조다.
- **11px 미만 글자 선언 433곳**, 최소 5.5px. 한글이 그 크기에서 뭉개진다.
- 색 리터럴 2106종 / 토큰 11개. z-index 51종(-2~9999). 이징 53종. 브레이크포인트 24종.

즉 텍스트 개수는 증상이고, 위계 부재·판독 불가 타이포·디자인 시스템 부재가 원인이다.
**P0는 화면을 바꾸지 않는다** — `tokens.css` + CSS `@layer` 래핑 + 회귀 방지 테스트만 깐다.
이 단계를 건너뛰면 다음 세션이 다시 무너뜨린다.

이 항목은 아래 VISUAL RESET 기준을 취소하지 않는다. 순서는 사용자 결정 사항이며 미결정.
