...
## 최상위 기획 · 2026-09-22 — V12 마스터피스 (제품 전체)

**읽는 순서: [`docs/V12-MASTERPIECE.md`](./docs/V12-MASTERPIECE.md) → [`docs/V12-UI-MASTER-SPEC.md`](./docs/V12-UI-MASTER-SPEC.md) → `docs/design/v12/`**
검수·수정은 아스트라. **승인 전 구현 금지.**

발단: 2026-09-22 플레이테스트. 연출·게임감은 개선됐고 이해도가 막힌다. 1스테이지 패배, "너무 복잡 어려움".
사용자 방향: 텍스트 줄이고 그래픽·효과로 치환. **게임 방식은 그대로.**

### 핵심 원리 — 규칙을 비유가 아니라 물리로 번역한다

아이콘도 결국 배워야 하는 기호다. 물리는 배우지 않아도 안다. 다섯 개의 번역이 이 기획의 척추다.

1. **커버 = 카드가 존을 덮는다.** 카드의 물리적 형태가 곧 커버 범위다
   (`cards.js`의 `shape`: point/column/row/cross/all → 1칸 정사각 / 세로로 긴 카드 / 가로로 납작한 카드 / 십자 / 9칸 전면).
   `COVER 5/9`는 사라지는 게 아니라 **존재할 이유가 없어진다.**
2. **여러 장 = 겹쳐 쌓인다.** 더미가 두꺼워질수록 예상 피해 구간이 짧아진다.
3. **이어치기 = 카드끼리 맞닿는다.** 붙으면 접합선이 빛나고 안 붙으면 떨어져 있다. `CONNECT`/`BREAK` 불필요.
4. **준비 카드는 카드가 아니다.** 원형 칩으로 만들면 존에 못 놓는 것이 형태로 자명하다.
5. **위력 = 카드의 무게.** `power` 0~3이 두께·소리·진동이 된다.

**엔진 변경 없음.** 위 전부 이미 있는 필드(`shape`/`power`/`pressure`/`gives`/`needs`)를 읽기만 한다.

### "정타 노림"이라는 글자가 필요한가

전투에서는 불필요, 덱·보상·도감에서는 필요. 다만 진짜 문제는 이름이 아니었다 —
카드 1장이 최대 **9행**을 들고 있고 이름은 그중 한 줄이다.
**이름을 지우는 게 아니라 이름만 남기는 것**이 답이다(9행 → 1행).

### 범위

전투 화면만이 아니라 **타이틀 · 지도(경로) · 전투 · 판정 · 보상 · 시설 · 덱/도감 · 패배/승리 전부**.
지도는 읽는 표가 아니라 보는 길로 바꾼다(노드 40행 이상 → 6 이하).
학습은 설명 모달이 아니라 **사건이 가르친다**(개념 10개의 첫 등장 시점 설계).

### 로드맵 P0~P8

P0 바닥(화면 변화 0) → P1 글자를 읽히게 → P2 카드를 물건으로 → P3 보드를 판으로 →
P4 연출이 가르치게 → P5 길을 보이게 → P6 비교를 형태로 → P7 말 없이 배우게 → P8 끝을 보이게.
**P0를 건너뛰면 이후가 전부 `!important`가 된다.**

### 시안 (docs/design/v12/)

- `card-shape-language.png` — 카드 16종 형태 언어 + 보드 덮기/맞닿기 데모
- `three-directions.png` — 전투 화면 3방향(A 타자 시점 / B 전광판 / C 덱 테이블)
- `before-after-portrait.png` — 현재 빌드 실캡처 대 시안
- `dir-a/b/c.html`, `after-portrait-390.html`, `tokens.css` — 동작하는 코드

### 실측 근거 (규격 §1.0)

실제 빌드를 Chromium 390×844로 조작해 계측: 보이는 텍스트 **211개**, 11px 미만 **186개(88%)**,
최소 5.5px, 문서 높이 **1.95뷰포트**, **투수 HP가 9존 조작 중 화면 밖**, 배경색 59종/글자색 50종.
CSS는 21파일 5258줄에 `!important` 2266개(전체 줄의 19.5%).

### 결정 필요 (기획 §14)

시각 방향 A/B/C · 카드가 칸을 물리적으로 덮을지 · 카드 이름 재명명 여부 ·
전투에서 이름을 감출지 · 단계 사이 톤 불일치 감수 여부 · 준비=원형 칩 여부 ·
**VISUAL REBOOT(아트)와의 선후**.

이 기획은 아래 VISUAL RESET 기준을 취소하지 않는다. 방향 A를 고르면 아트 리부트가 **선행 조건**이 된다.
