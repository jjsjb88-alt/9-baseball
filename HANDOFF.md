...
## 새 피드백 · 2026-09-22 — 초보 이해도

실플레이 테스터 피드백이 들어왔다. 요지는 **연출·게임감은 개선됐고, 이해도가 막힌다**다.
1스테이지에서 패배, "너무 복잡 어려움", "야구 초보나 겜초보에겐 좀더 직관적이어야 할듯".

계획서: [`docs/V12-ONBOARDING-PLAN.md`](./docs/V12-ONBOARDING-PLAN.md).
가장 큰 원인은 **MAIN RUN에서 웰컴 가이드가 아예 실행되지 않는 것**이다(`src/duel/App.jsx:719`의 `isV10` 제외).

**이 항목은 아래 VISUAL RESET 기준을 취소하지 않는다.** 아트 루프와 온보딩 중
무엇을 먼저 할지는 사용자 결정 사항이며, 아직 결정되지 않았다. 기획안은 작성만 했고
**구현은 시작하지 않았다.**
