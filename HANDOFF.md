...
- 브랜치 `claude/feedback-review-plan-vw24at` (main 병합·배포 없음). 단일 기준: [docs/V12-PROGRESS.md](docs/V12-PROGRESS.md) — 줄마다 영향도·검증·캡처·SHA.
- D1~D6 사용자 확정(2026-09-24). D7(아트 병행)·D8(범위)은 권고만.
- 전투(P1) · 카드 상세/준비 토큰/커버 미리보기(P2) · 메인·지원 커버/순서 링크/순서 시트/효율 표기(P3) · 결과 이유 사슬·다음 버튼 고정(P4) · 지도 상세 고정·비교(P5) · 보상 선택 결과·투수 지문 근거화(P6) · 도움말 사실화·가이드·저장 경고(P7) · 종료 기록·도감(P8).
- 이 과정에서 찾아 고친 **기존 버그**(배포본에도 있을 가능성, 반영은 사용자 판단): 지켜본 공마다 저장 실패(`16d88cb`), 라커룸 뒤 1막 전투 흰 화면(`4de7886`), 가로 라커룸 앞쪽 카드 도달 불가(`9a2a98d`), 가로 보상 버튼 줄의 떠돌이 READ 버튼(`c67eda3`), 지도 투수 지문이 근거 없는 표(`6cf444b`).
- 검증 도구: 전체 흐름 스모크(새 런→지도→전투→보상→라커룸→전투→종료, 3뷰포트), 결과 11종 재생, 실터치(CDP). 스크립트는 세션 scratchpad에 있었으므로 필요하면 PROGRESS의 방법 설명으로 재구성.
- 남은 한계: 원화(배우-경기장 밀도, 정면 투수), 작은 세로(375×667) 스크롤, 카드 위 세로 밀기=끌기(실기기 확인 필요), 정적 font-size 선언 정리(C7), 이전 저장 지도의 지문 미표시.
...
최신 사용자 판정:
- 기존 A/B/C는 모두 거절.
- D/E/F 중 E/F 계열의 감각은 괜찮다는 반응.
- 그러나 결과물 최종 판정은 **“마스터피스가 아닌데?”**
- 따라서 **D1 미확정 / P1-1 미착수** 상태를 유지한다.

다음 세션은 E/F를 단순 합성하거나 제품에 구현하지 않는다.
E의 몰입감 + F의 정보 구조 원리만 참고해 **Golden Master 1안**을 먼저 만든다.
목표는 “UI가 깔끔한 야구게임”이 아니라 **스크린샷 한 장만으로 기억되는 9ZONE 대표 전투 화면**이다.

저품질 배우 placeholder는 결정용 시안에서도 금지한다.
SVG/PNG 같은 포맷은 품질 기준이 아니며, 실루엣·인체비례·키포즈·체중이동·3단 명암·장비 가독성·축소 판독성·환경 통합감이 품질 게이트다.
...
## V12 루프 브랜치 비교 판정 · 2026-09-24

- `claude/feedback-review-plan-vw24at`(PR #83, V12 P0~P8) vs main을 같은 시드로 390×844 / 844×390 / 1440×900 타이틀·지도·전투 스크린샷 비교.
- **판정: main 유지, V12 통째 병합 보류.** V12는 #88 이전에서 갈라져 세 뷰 모두 전투에 실제 타자가 없음(배트·홈플레이트 대체물), 가로에서 `BATTER VS PITCHER`·READ 라벨 겹침, 카드 줄 잘림. 장점(12px 글자 하한, 지도 상세 고정 시트)은 있으나 시각 회귀가 더 큼.
- V12에서 독립 버그 수정 2개만 main 쪽으로 이식: 지켜본 공 저장 실패(`16d88cb`), 가로 보상/라커룸 줄 첫 카드 도달 불가(`9a2a98d`). 라커룸 빈 화면은 #85로 이미 main에 있음.
- V12의 나머지(P2~P8 UX)는 main 최신 위에서 화면 단위로 다시 가져올 후보. 통째 병합 시 App.jsx/engine.js/main.jsx 등 6파일 충돌.
...
핵심 수정:
- 커버는 shape 고정 그림이 아니라 coverageAt의 실제 셀. 십자 모서리/확장/성장 예외 포함.
- 연결은 카드 접촉이 아니라 순서별 aimZone 동일/8방향 인접. 메인·순번·편집 경로 보존.
- damageRate는 피해 배율, 실제 예상 HP가 아님. 읽기 등급은 0/1/2.
- 이름·비용·조건·오류·공개 정보 경계를 보존하며 중복 장식부터 줄임.
- P0 일괄 CSS 레이어 래핑·임의 important 감축 제거. 컴포넌트별 검증으로 이관.
- native 스크롤·탭·키보드 보존, 접근성/학습 검증을 후반으로 미루지 않음.
- 제품 전체 범위와 P0~P8 통일. 아트 기준은 아래 VISUAL RESET 그대로 유지.

시각 방향 권고: C의 정면 보드/큰 손패 + B의 정보 위계, 판정에서는 기존 경기장/배우 연출 보존.
기존 PNG/HTML은 **수정 전 정적 개념 자료**이며 구현 정답이나 시각 PASS 증거가 아니다.
