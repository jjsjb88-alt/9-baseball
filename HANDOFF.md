...
- `src/duel/presentation.js`가 READ / HIT / EXTRA / HOMER / WHIFF / FOUL / BALL / CALLED / OUT / SACRIFICE 및 준비 카드 연출의 단일 의미 표다.
- `src/duel/duel.css`의 `MASTER PRESENTATION` 절은 스캔, 충격 링, 카메라 충격, 존 적중/미스 비교, 공 궤적, 프리즈를 담당한다.
- `src/duel/audio.js`는 판정별 합성 사운드를 갖는다. 사운드 실패가 전투를 막으면 안 된다.
...
- 제품 코드+연출 CSS 기준 GitHub Actions run `34955006884` 성공.
- 자동 수치는 종료·저장·난도 회귀 기준이지 인간 재미·연출 완성도 증명이 아니다.

다음 우선순위:
1. 실제 모바일/데스크톱에서 READ → 맞춤 → 빗나감 → 홈런 연출을 사람 눈으로 검수한다.
2. 판정이 반복될 때 0.88~1.12초의 잠금 시간이 쾌감인지 피로인지 조정한다.
