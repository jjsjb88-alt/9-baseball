...
## 새 피드백 · 2026-09-22 — 전투 화면 UI/UX 마스터 규격 + 실측 + 시안

테스터: 연출·게임감은 개선됐고 **이해도가 막힌다.** 1스테이지 패배, "너무 복잡 어려움".
사용자 방향: **텍스트 줄이고 그래픽·효과로 치환. 게임 방식은 그대로.**

규격: [`docs/V12-UI-MASTER-SPEC.md`](./docs/V12-UI-MASTER-SPEC.md) · 시안: `docs/design/v12/`
검수·수정은 아스트라. **승인 전 구현 금지.**

### 실제 빌드를 띄워 계측한 결과 (규격 §1.0)

Chromium 390×844로 새 런 → 1막 정규 승부 → 스윙 → 카드 3장 배치까지 조작 후 측정.

- **보이는 텍스트 노드 211개, 그중 11px 미만 186개(88%), 최소 5.5px.** 12px 이상은 25개뿐.
- **문서 높이 1646px = 뷰포트의 1.95배.** 한 화면에 안 들어온다.
- **투수 HP(y152)는 9존(y505~869)을 조작하는 스크롤 위치에서 화면 밖이다.**
  카드를 놓으면서 그 결과인 HP를 볼 수 없다.
- 9존 하단과 손패 상단 사이 194px. 둘이 동시에 안 보여 **배치마다 스크롤 왕복**.
- 한 화면에 배경색 59종 / 글자색 50종.
- 대비비는 5건 중 4건 통과 — **안 읽히는 원인은 대비가 아니라 크기다.**

### 실렌더에서 확인된 기존 버그 3건 (이 기획과 별개로 이미 깨져 있음)

1. 배치 토큰이 셀 경계를 넘어 옆 칸 글자를 덮는다.
2. **가로모드 좌하단에서 `BATTLE READOUT`·`SWING`·`COVER`·`COACH` 블록이 서로 포개진다.** 카드도 잘린다.
3. 세로모드 우하단 `선수는 베이스에 · 카드는 행동에`가 화면 밖으로 잘린다.

2번은 `!important` 2266개(전체 CSS 줄의 19.5%)가 만든 결과의 눈에 보이는 증거다.

### 시안 (docs/design/v12/)

같은 게임 상태를 규격대로 그린 결과: **텍스트 211→16, 11px 미만 186→0, 1.95뷰포트→1.00.**
`after-portrait-390.html`은 정지 이미지가 아니라 **토큰·해치·아크가 실제로 동작하는 코드**다.

한계: 정지 1장이라 모션(규격 §8)은 미증명. **타자/투수 아트 품질은 시안으로 해결되지 않는다** —
현재 빌드의 타자는 배트와 홈플레이트만 있는 대역이고, 시안은 기존 스프라이트를 되살려 놓았을 뿐이다.
아래 VISUAL RESET 기준은 그대로 유효하며, 순서는 사용자 결정 사항이다.
