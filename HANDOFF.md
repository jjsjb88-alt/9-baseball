...
## 최우선 인수인계 V9.1 · 2026-09-15

현재 개발 브랜치는 `codex/v9-deckbuilder`, 제품 계약은 `docs/V9.md`다.
**main / 공개 Pages는 V8.10 안정본 그대로이며 V9.1은 아직 배포하지 않았다.**

해석을 바꾸지 말 것:
- V8.10 세 덱은 초기덱이 아니라 완성형 체험/튜토리얼.
- MAIN RUN은 9장 공통 스타터 → 경기 카드 드래프트 → 경기 사이 시설 선택으로 성장한다.
- 카드 보상은 3장 중 1장 또는 건너뛰기.
- 시설은 2택1이며 모든 기능을 한 런에 다 얻지 못한다.
- 훈련=강화, 라커룸=제거, 장비실=유물, 스카우팅=다음 한 경기 읽기 +1.
- 스카우팅은 pending 실제 공을 절대 공개하거나 재추첨하지 않는다.
- 커버 적중=안타 계약 유지. 적중 후 실패 확률을 되살리지 않는다.
- 기다림/연결/행운 성장을 MAIN RUN에 다시 강제로 붙이지 않는다.

검증:
- 114/114 테스트.
- V9.1 30시드 full-run 7/30 완주, 25/30 첫 보상 이상 도달.
- 모든 시설 선택과 중간 상태 저장/복원.
- 기존 zone / 성장 / 덱·유물 리포트와 프로덕션 빌드 성공.
- 자동 수치는 종료·저장·난도 회귀 기준이지 인간 재미 증명이 아니다.

다음은 V9.2: 라이벌/강팀의 고위험 고보상 노드와 실제 브랜칭 맵.
그 전에 인간 플레이로 시설 선택이 실제 고민인지, 스카우팅이 필수 편의가 아닌지 확인한다.
