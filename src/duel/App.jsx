...
import {RED_RUSH_ASSET_ID,hasPitchVisual,redRushFrameAt,redRushTimeline} from './pitcher-sd.js';
...
function Sprite({who,stage=null,shot=null,golden=false,pitcherAsset=null,pitchToken=0}){
  const redRush=who==='pitcher'&&pitcherAsset===RED_RUSH_ASSET_ID;
  const pose=actorPose(who,stage,shot),spec=redRush?null:sequenceSpec(who,stage,shot),animated=useSpriteFrame(spec,who+'-'+stage+'-'+(shot?.grade||'idle'));
  const reduced=typeof window!=='undefined'&&!!window.matchMedia?.('(prefers-reduced-motion: reduce)').matches;
  const redFrame=useRedRushFrame(redRush&&!!stage&&hasPitchVisual(shot)&&!reduced,pitchToken);
  const col=redFrame%10,row=Math.floor(redFrame/10);
  const atlasStyle=redRush?{backgroundImage:'url('+redRushAtlas+')',backgroundSize:'1000% 1200%',backgroundPosition:(col*100/9)+'% '+(row*100/11)+'%'}:null;
  const legacy=(who==='pitcher'?PITCHER_POSES:BATTER_POSES)[pose],v2=golden?V2_FALLBACKS[who]?.[pose]?.():null,src=animated||v2||legacy;
  const actorImage=className=>redRush?<span aria-hidden="true" className={className+' red-rush-frame'} style={atlasStyle}/>:<img aria-hidden="true" className={className} src={src}/>;
  return <span className={'sprite-stage sprite-'+who+' pose-'+pose+(animated||redRush?' v2-sequence':'')+(golden?' golden-actor':'')+(redRush?' red-rush-actor':'')}>
    {golden&&<i className="actor-ground" aria-hidden="true"/>}
    {golden&&<i className="actor-contact-shadow" aria-hidden="true"/>}
    {actorImage('sprite-echo echo-back')}
    {actorImage('sprite-echo echo-mid')}
    {actorImage('duel-sprite')}
...
function CinemaLab({sound,onBack}){
  ...
  const timers=useRef([]),shot=presentationFor(chosen.state),baseTimeline=presentationTimeline(shot,false),timeline=hasPitchVisual(shot)?redRushTimeline(baseTimeline):baseTimeline,revealed=chosen.state.battle.revealed;
  useEffect(()=>()=>timers.current.forEach(clearTimeout),[]);
  function play(item=chosen){
    timers.current.forEach(clearTimeout);setChosen(item);setToken(x=>x+1);setStage('windup');
    const p=presentationFor(item.state),base=presentationTimeline(p,false),t=hasPitchVisual(p)?redRushTimeline(base):base,pitch=!!item.state.battle.revealed;
  ...
  return <main className={'cinema-lab release-combat stadium-stage-3 rival-game'+(shot?' fx-'+shot.kind:'')+(shot?.motion?.shake?' shake-'+shot.motion.shake:'')+(stage?' fx-stage-'+stage:'')}>
    ...
    <section className={'cinema-lab-stage duel-arena stadium golden-master-stage '+(renderer2?'renderer2-host ':'renderer1-host ')+arenaPresentationClass(stage,shot)} aria-label="연출 검수 무대">
      ...
      <div className="actor-left"><Sprite who="batter" stage={stage} shot={shot} golden/><span className="batter-nameplate">#09 TEST BATTER</span></div><div className="actor-right"><Sprite who="pitcher" stage={stage} shot={shot} golden pitcherAsset={RED_RUSH_ASSET_ID} pitchToken={token}/><span className="pitcher-nameplate">레드 러시 · 120F</span></div>
