...
function GrowthSummary({s}){
  if(s.build===DECKBUILDER_BUILD)return <div className="growth-summary" aria-label="덱 빌딩 기록"><span>MAIN RUN · 덱 {s.deck.length}장 · 보상 {s.rewards.length}/3</span></div>;
...
function GrowthPanel({s,disabled,onMode}){
  const b=s.battle;
  if(s.build===DECKBUILDER_BUILD)return <div className="growth-notice">V9 MAIN RUN · 성장 트리 대신 카드와 유물로 이번 런의 타격 스타일을 만듭니다.</div>;
