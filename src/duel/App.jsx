...
import {createV10Duel,enterV10Node,playV10Action,advanceV10Pitch,advanceV10Batter,claimV10Reward,
  ...
  saveV10Duel,readV10Duel,previewV10Stack,V10_SWING_STACK_MAX,V10_SWING_DAMAGE_RATES,V11_STACK_CONNECT_BONUS} from './engine.js';
...
import PitcherPortraitDialog from './PitcherPortraitDialog.jsx';
...
import batterSwingV4 from '../../assets/sprites-v4/batter-swing-60.png';
import batterMissV4 from '../../assets/sprites-v4/batter-miss-60.png';
...
function PitcherAtlasSprite({atlas,stage=null,shot=null,playToken=0}){
  ...
  const atlasStyle={backgroundImage:'url('+atlas+')',backgroundSize:'1000% 1200%',backgroundPosition:(col*100/9)+'% '+(row*100/11)+'%'};
...
function CinemaLab({sound,onBack}){
  ...
  return <main className={'cinema-lab release-combat stadium-stage-3 rival-game'+(shot?' fx-'+shot.kind:'')+(shot?.motion?.shake?' shake-'+shot.motion.shake:'')+(stage?' fx-stage-'+stage:'')}>
    ...
    <section className={'cinema-lab-stage duel-arena stadium golden-master-stage '+(renderer2?'renderer2-host ':'renderer1-host ')+arenaPresentationClass(stage,shot)} aria-label="연출 검수 무대">
      ...
      <div className="actor-left"><Sprite who="batter" stage={stage} shot={shot} golden playToken={token}/><span className="batter-nameplate">#09 TEST BATTER</span></div><div className="actor-right"><Sprite who="pitcher" stage={stage} shot={shot} golden playToken={token}/><span className="pitcher-nameplate">TEST PITCHER</span></div>
...
function ZoneBoard({s,id,onZone,disabled,coveredOverride=null,matchupOverride=null,coverMain=null,coverSupport=null,links=null}){
  ...
  // V12 P3-1: before the reveal, main and support cover are told apart (engine primary/support coverage)
  const mainCover=b.revealed?[]:coverMain||covered,supportCover=b.revealed?[]:coverSupport||[];
  ...
  return <section className="zone-panel" aria-label="9존 타격 계획">
    ...
    <div className="zone-grid" role="group" aria-label="노릴 코스" aria-describedby="zone-keyboard-help">
      {ZONES.map((name,z)=>{
        ...
        const inMain=mainCover.includes(z),inSupport=supportCover.includes(z);
        return <button key={z} ref={el=>zoneButtons.current[z]=el} aria-label={name} aria-describedby={'zone-read-'+z+(inMain||inSupport?' zone-cover-'+z:'')} title={title} aria-pressed={b.aimZone===z} disabled={disabled}
          tabIndex={b.aimZone===z?0:-1} onKeyDown={e=>moveZone(e,z)} onClick={()=>onZone(z)}
          className={'zone-cell shade-'+shadeFor(share(z))+' '+((dead&&showUnused)||ruledOut?'unused ':'')+(covered.includes(z)?'covered ':'')+(inMain?'cover-main ':'')+(inSupport?'cover-support ':'')+(b.aimZone===z?'aimed ':'')+(b.revealed?.zone===z?'actual':'')}>
          <span>{name}</span><strong id={'zone-read-'+z}>{figure}</strong><small>{proficient.includes(z)?'★ 숙련':'비숙련'}{b.aimZone===z?' · 노림':''}</small>
          {(inMain||inSupport)&&<span id={'zone-cover-'+z} className="sr-only">{[inMain&&'메인 커버',inSupport&&'지원 커버'].filter(Boolean).join(' · ')}</span>}
        </button>;
      })}
      {!b.revealed&&<ZoneLinks links={links}/>}
    </div>
...
export default function Duel(){
  ...
  // PLAN -> WATCH THE BUILD EXECUTE -> REVIEW. It never overwrites the saved main run.
  const [autoLesson,setAutoLesson]=useState(false),[autoPlan,setAutoPlan]=useState(null);
  /* V13: the main run (v10) always plays on the ballpark screens; the legacy screens remain only for the tutorial runs (v9). */
  ...
  const [orderOpen,setOrderOpen]=useState(false),orderOpener=useRef(null);
  const closeOrder=()=>{setOrderOpen(false);const el=orderOpener.current;orderOpener.current=null;setTimeout(()=>{const t=el?.isConnected?el:document.querySelector('[data-testid=execute-action]');t?.focus();},0);};
  const closeCardDetail=()=>{setCardDetail(null);const el=detailOpener.current;detailOpener.current=null;setTimeout(()=>{if(el?.isConnected)el.focus();},0);};
  ...
  const prepLeft=b?Math.max(0,2-b.preparations):0,prepUses=entry=>CARDS[entry.kind].type==='skill'?'준비 '+prepLeft+'회 남음':null;
  ...
  const selectedDetailName=selected==='basic'?'BASIC SWING':selectedEntry?CARDS[selectedEntry.kind].name+(selectedEntry.plus?'+':''):null;
  ...
  const rawChoice=selected&&(selected==='basic'||b?.hand.includes(selected))
    ?(isV10&&(decisionMode==='swing'||CARDS[selectedEntry?.kind]?.type!=='skill')&&selectedEntry?previewV10Stack(s,selected,activeStack):showPreview(selected)):null;
  ...
  const pitcherAtlas=isV10?pitcherAtlases[v10Node?.opponent?.artId]:null;
  const redRushEncounter=!!pitcherAtlas;
  ...
  useEffect(()=>()=>timers.current.forEach(clearTimeout),[]);
  useEffect(()=>installCoverPreview(document),[]);
  ...
  function clearActionUi(){setGrowthChoice(null);setRewardAction(null);setFacilityChoice(null);setStackResolve(null);setModal(null);setSelected(null);setSwingStack([]);setStackEdit(null);setDecisionMode(null);setTour({open:false,step:0});}
  function freshV10(){setAutoLesson(false);setAutoPlan(null);clearActionUi();persist(createV10Duel(mainRunSeed()));setScreen('run');}
  function fresh(){
    setAutoLesson(false);setAutoPlan(null);clearActionUi();
    persist(createDuel(Number(trialSeed)>>>0,build));setScreen('run');
  }
  function freshAutoLesson(){
    clearActionUi();
    let next=createV10Duel(Number(trialSeed)>>>0);
    // Skip the map only for this sandbox so the idea is visible immediately.
    // A reachable node with an opponent is a combat node; if the map shape ever
    // changes, the fallback simply leaves the player on the normal map.
    const first=(next.runMap?.reachableIds||[])
      .map(id=>next.runMap?.nodes?.find(n=>n.id===id))
      .find(n=>n?.opponent);
    if(first)next=enterV10Node(next,first.id);
    current.current=next;setS(next);setError('');setAutoPlan(null);setAutoLesson(true);setScreen('run');
  }
  function exitAutoLesson(){
    timers.current.forEach(clearTimeout);timers.current=[];lock.current=false;
    setFx(null);setFxStage(null);setFrame(0);setStackResolve(null);setAutoPlan(null);setAutoLesson(false);
    const restored=initial.save||null;current.current=restored;setS(restored);setScreen('menu');setModal(null);setSelected(null);setSwingStack([]);setStackEdit(null);
  }
  function captureAutoPlan(kind){
    if(!autoLesson)return;
    if(kind==='take'){setAutoPlan({kind:'take',cards:['지켜본다'],zones:[],coverage:0,damageRate:null});return;}
    const ids=[selected,...activeStack.map(x=>x.id)].filter(Boolean);
    const cards=ids.map(id=>id==='basic'?'맨손 스윙':CARDS[byId(id)?.kind]?.name||'스윙');
    const zones=[selected?b?.aimZone:null,...activeStack.map(x=>x.aimZone)].filter(Number.isInteger).map(v10ZoneName);
    setAutoPlan({
      kind,
      cards,
      zones,
      coverage:choice?.coverage?.length||0,
      damageRate:choice?.damageRate??1,
      links:choice?.stackPlan?.links?.length||0,
    });
  }
  ...
  return <div className={'duel-app diamond-app'+(fxPresentation?' presentation-'+fxPresentation.kind:'')+(fxStage?' presentation-stage-'+fxStage:'')+(stackResolve?' stack-resolving':'')}>
...
:showBattle&&isV10?<BallparkBattle key="screen-bp-battle" s={s} hand={hand} selected={selected} swingStack={swingStack} choice={choice} locked={!!stackResolve}
    ...
    :showBattle?<main key="screen-battle" data-perf-tier={perfTier} className={'duel-combat release-combat perf-'+perfTier+' stadium-stage-'+s.stage+' phase-'+s.phase+' decision-'+(decisionMode||'hub')+(isV10&&s.phase==='battle'&&!fxStage&&(decisionMode==='swing'||decisionMode==='prepare')?' v12-design':'')+(isV10&&(s.phase==='pitch'||s.phase==='between')&&!fxStage?' v12-result':'')+(fxStage?' cinematic-focus':'')+(routeChoice(s.stage,s.route)?.statBonus?' rival-game':'')+(fxPresentation?' fx-'+fxPresentation.kind:'')+(fxStakes?' stakes-'+fxStakes:'')+(isV10&&fxPresentation?.motion?.shake?' shake-'+fxPresentation.motion.shake:'')+(fxStage?' fx-stage-'+fxStage:'')}><div className="battle-ribbon"><span>{isV10?v10Node?.act+'막':s.stage+1+' / 4'}</span><b>{isV10?(v10Node?.name||'승부'):STAGES[s.stage].name}</b><span>타석 {b.turn}</span></div>{isV10&&v10Pitcher&&<div className="v10-combat-hp"><PitcherHpHud {...v10Pitcher} name={(v10Node?.opponent?.archetype?v10Node.opponent.archetype+' · ':'')+v10Pitcher.name}/>{v10Combat&&<CombatResultSummary choice={{card:v10Combat.choiceLabel||v10ChoiceName(v10Combat.choice),zone:v10Combat.aimLabel||v10ZoneName(b.revealed?.aimZone)}} actualPitch={{type:v10Combat.pitchName||b.intent?.name,zone:v10Combat.pitchLabel||v10ZoneName(v10Combat.actualPitch)}} verdict={v10Combat.verdict} damage={v10Combat.damage} hpAfter={v10Combat.hpAfter} cardCount={v10Combat.cardCount||1} damageRate={v10Combat.damageRate??1}/>}</div>}<Scoreboard s={s} onLineup={()=>setModal('lineup')} targetRef={scoreboardRef}/><section ref={arenaRef} className={'duel-arena stadium renderer2-host golden-master-stage pitcher-form-'+pitcherForm+' '+arenaPresentationClass(fxStage,fxPresentation)+(fx?.kind==='hit'?' impact':'')+(s.phase==='between'?' between':'')} aria-label="승부 구장"><ArenaRenderer2 stage={fxStage} shot={fxPresentation} match={s.stage} rival={!!routeChoice(s.stage,s.route)?.statBonus} revealed={b.revealed} token={s.stats.pitches} label="실전 WebGL 경기장" quality={perfTier}/><GoldenMasterStage stage={fxStage} shot={fxPresentation} rival={!!routeChoice(s.stage,s.route)?.statBonus} match={s.stage}/>{stackResolve&&<StackResolve plan={stackResolve.plan} token={stackResolve.token}/>}{isV10&&<><PixelCinema stage={fxStage} shot={fxPresentation}/><PixelVFX stage={fxStage} shot={fxPresentation} token={s.stats.pitches} drawCore={false} quality={perfTier}/></>}<ReadTrace s={s} stage={fxStage} shot={fxPresentation}/><div className={'intent '+b.intent.kind}><span>{s.phase!=='battle'?'판정 완료':'투수 의도'}</span><strong>{b.intent.name}</strong><b>{b.revealed?'실제 공 공개':'코스 경향 · 확정 예고 아님'}</b></div><div className={'actor-left '+(s.phase==='between'&&!fx?'leaving-batter':'')} key={'batter-'+b.turn}><Sprite who="batter" stage={fxStage} shot={fxPresentation} golden playToken={s.stats.pitches} syncRedRush={redRushEncounter}/><span className="batter-nameplate">#{currentBatter(s).number} {currentBatter(s).name}</span></div><div className={'actor-right pitcher-form-'+pitcherForm}>{redRushEncounter?<PitcherAtlasSprite atlas={pitcherAtlas} stage={fxStage} shot={fxPresentation} playToken={s.stats.pitches}/>:<Sprite who="pitcher" stage={fxStage} shot={fxPresentation} golden variant={pitcherForm} playToken={s.stats.pitches}/>}{isV10&&<span className="pitcher-nameplate">{v10Pitcher?.name||'OPPONENT'}</span>}{isV10&&pitcherPortraits[v10Node?.opponent?.artId]&&<button type="button" className="pitcher-inspect-hitbox" aria-label={(v10Pitcher?.name||v10Node.opponent.name)+' 투수 큰 그림 보기'} onClick={e=>openPitcher(v10Node.opponent,e)}><span aria-hidden="true">↗</span></button>}</div><Diamond s={s}/>{fx&&<FlightVisual key={'effect-'+s.stats.pitches} revealed={b.revealed} fx={fx} showCopy/>}{fxPresentation&&<JudgementVisual key={'judgement-'+s.stats.pitches+'-'+fxPresentation.kind} shot={fxPresentation} stage={fxStage} cinema={isV10}/>}{isV10&&<ClutchLayer stakes={fxStakes} stage={fxStage}/>}</section><ZoneBoard s={s} id={selected||"basic"} coveredOverride={activeStack.length?choice?.coverage:null} links={activeStack.length?choice?.stackPlan?.links:null} coverMain={activeStack.length?choice?.primaryCoverage:null} coverSupport={activeStack.length?(choice?.supportCoverages||[]).flatMap(x=>x.coverage):null} matchupOverride={activeStack.length?choice?.matchup:null} disabled={s.phase!=='battle'||!!fx||tour.open} onZone={z=>{if(!lock.current)persist(setAimZone(current.current,z));}}/><div className="pitch-read"><span>{s.phase!=='battle'?'RESULT':'READ'}</span><b>{s.phase!=='battle'?s.last.text:b.intent.detail}</b>{s.phase==='battle'&&<small>집중 +{b.aim} · 준비 {b.preparations}/2 · 손패 {b.hand.length}</small>}</div><section className="duel-table">{!isV10&&<GrowthPanel s={s} disabled={!!fx||tour.open||s.phase!=='battle'} onMode={toggleGrowth}/>}<div ref={eventsRef} className={'diamond-events '+(s.last?.kind==='repertoire'?'repertoire-event':'')} role="status"><b>{s.last?.text}</b>{s.last?.events?.slice(0,2).map((e,i)=><span key={i}>{e}</span>)}{s.last?.growthEvents?.map((e,i)=><b key={'g'+i} className="growth-event">{e}</b>)}</div>{s.phase==='pitch'&&<section className={'pa-result result-'+(b.revealed?.kind||'neutral')+' result-grade-'+(resultPresentation?.grade||'neutral')} aria-label="투구 결과"><span>REVEAL · SAME BATTER</span><strong className="result-call">{resultPresentation?.title||resultCall(b.revealed)}</strong><h2>{b.revealed.label}</h2>{isV10&&<ResultChain revealed={b.revealed} combat={v10Combat}/>}<p>{b.balls}볼 {b.strikes}스트라이크 · {currentBatter(s).name} 그대로 타석에</p><FailureTimeline s={s}/>{isV10&&b.turn<=2&&v10Combat&&<DecisionDebrief combat={v10Combat} revealed={b.revealed}/>}<button className="primary next-batter" disabled={!!fx} onClick={()=>act(doNextPitch)}>다음 공 · 같은 타자</button></section>}{s.phase==='between'&&<section className={'pa-result result-'+(b.revealed?.kind||'neutral')+' result-grade-'+(resultPresentation?.grade||'neutral')} aria-label="타석 종료 결과"><span>PLATE APPEARANCE COMPLETE</span><strong className="result-call">{resultPresentation?.title||resultCall(b.revealed)}</strong><h2>{b.revealed?.label}</h2>{isV10&&<ResultChain revealed={b.revealed} combat={v10Combat}/>}<p>{b.batterIndex+1}번 {currentBatter(s).name} · 타석 종료</p><p className="runner-summary">현재 주자 · {b.bases.map((id,i)=>id?(i+1)+'루 '+LINEUP.find(p=>p.id===id).name:null).filter(Boolean).join(' / ')||'없음'}</p><FailureTimeline s={s}/>{isV10&&b.turn<=2&&v10Combat&&<DecisionDebrief combat={v10Combat} revealed={b.revealed}/>}<button aria-label={`다음 타자 입장 · ${(b.batterIndex+1)%9+1}번 ${LINEUP[(b.batterIndex+1)%9].name}`} className="primary next-batter" disabled={!!fx} onClick={()=>act(doNextBatter)}>다음 타자 입장 · {(b.batterIndex+1)%9+1}번 {LINEUP[(b.batterIndex+1)%9].name}<span>NEXT BATTER →</span></button></section>}{s.phase==='battle'&&<>{!decisionMode?<section className={'decision-hub '+(firstDecision?'coach-focus':'')} aria-label="이번 타석 행동 선택"><div className="decision-copy"><span>{firstDecision?'첫 타석 가이드':'YOUR CALL'}</span><h2>{firstDecision?'투수 의도를 읽었습니다. 이제 하나만 선택하세요.':'이번 공, 어떻게 가져갈까?'}</h2><p>{firstDecision?'9존을 선택하고 카드 커버를 확인하세요. 볼을 기다리거나, 넓게 버티거나, 좁게 장타를 노립니다.':'준비와 스윙을 함께 비교하세요. 한 공에 스윙은 한 번입니다.'}</p></div><div className="action-dock"><button ref={prepareRef} className="action prepare" aria-label="준비하기" disabled={b.preparations>=2||!prepareHand.length||!!fx} onClick={()=>openDecision('prepare')}><span>1</span><strong>준비하기</strong><small>{b.preparations}/2 · {prepareHand.length}장</small></button><button ref={swingRef} className="action swing" aria-label="스윙하기" disabled={!!fx} onClick={()=>openDecision('swing')}><span>2</span><strong>스윙하기</strong><small>{isV10?'카드를 존에 최대 '+V10_SWING_STACK_MAX+'장 배치':swingHand.length+'장 + 기본 스윙'}</small></button><button ref={watchRef} className="action watch" aria-label="한 구 지켜보기" disabled={!!fx} onClick={()=>openDecision('watch')}><span>3</span><strong>한 구 지켜보기</strong><small>{b.strikes===2?'스트라이크면 삼진 · 볼이면 생존':'볼 / 스트라이크 확인'}</small></button></div></section>:decisionMode==='watch'?<section className="watch-confirm"><h2>이 공을 지켜볼까?</h2><p>{b.strikes===2?'스트라이크면 삼진. 볼이면 볼카운트가 쌓입니다.':'볼은 볼카운트 +1, 스트라이크는 +1. 다음 공에 카드 1장을 뽑습니다.'}</p><button onClick={()=>setDecisionMode(null)}>돌아가기</button><button className="primary" onClick={()=>act(doTake,true)}>지켜보기 · 공 진행</button></section>:<section ref={drawerRef} className="card-drawer" data-main-exclusive={selectedEntry?.kind==='bunt'?'1':undefined} aria-label={decisionMode==='prepare'?'준비 카드 선택':'스윙 카드 선택'}><div className="drawer-head"><button className="drawer-back" onClick={()=>{setDecisionMode(null);setSelected(null);setSwingStack([]);setStackEdit(null)}}>← 돌아가기</button><div><span>{decisionMode==='prepare'?'PREPARE':'SWING'}</span><h2>{decisionMode==='prepare'?'스윙 전에 무엇을 준비할까?':'이 공을 어떻게 칠까?'}</h2></div><small>{visibleHand.length}장 · 손패 전체 {hand.length}장</small></div><div className="drawer-escape"><span>{b.strikes===2?'존 밖이면 볼, 존 안이면 삼진입니다.':'존 밖으로 올 것 같으면 치지 않아도 됩니다.'}</span><button className="drawer-watch" disabled={!!fx} onClick={()=>act(doTake,true)}>한 구 지켜보기</button></div>{isV10&&decisionMode==='swing'&&b.turn<=2&&<div className="stack-discovery" role="note" aria-label="카드 겹치기 안내"><div className="stack-fan" aria-hidden="true"><i/><i/><i/><i/></div><div><span>NEW · SWING STACK</span><strong>한 장만 낼 필요 없습니다.</strong><p>메인 카드를 고른 뒤 최대 3장을 더 겹치세요. <b>커버는 넓어지고</b>, 대신 <em>투수 HP 피해 효율은 내려갑니다.</em></p></div></div>}<div className="duel-hand">{decisionMode==='swing'&&<button className="duel-card basic-card" data-card-kind="basic" data-card-plus="0" data-cover-map={coverMapOf(s,'basic')||undefined} aria-keyshortcuts="i" aria-label="BASIC SWING" aria-pressed={selected==='basic'} onClick={()=>{setSelected('basic');setSwingStack([]);setStackEdit(null)}}><span className="card-cost">항상 사용</span><strong>BASIC SWING</strong><p>선택한 1존 · 카드 소비 없음</p>{b.growthMode==='patience'&&<small>기다림 성장으로 장타 가능</small>}<small>카드가 없어도 승부할 수 있습니다.</small></button>}{visibleHand.map(({id,entry,preview})=><Card key={id} kind={entry.kind} plus={entry.plus} uses={prepUses(entry)} coverMap={decisionMode==='swing'?coverMapOf(s,id):null} relation={relationOf(id)} selected={selected===id} problem={preview.problem} preview={preview} onClick={()=>{if(!lock.current){setSelected(id);setSwingStack([]);setStackEdit(null)}}}/>)}{!visibleHand.length&&<p className="empty-hand">{decisionMode==='swing'?'스윙 카드는 없지만 BASIC SWING은 사용할 수 있습니다.':'지금 사용할 수 있는 준비 카드가 없습니다.'}</p>}{otherHand.length>0&&<div className="hand-divider" aria-hidden="true"><span>{decisionMode==='prepare'?'스윙':'준비'}</span></div>}{otherHand.map(({id,entry,preview})=><Card key={id} kind={entry.kind} plus={entry.plus} uses={prepUses(entry)} relation={relationOf(id)} selected={selected===id} problem={preview.problem} preview={preview} note={(CARDS[entry.kind].type==='skill'?'준비':'스윙') + ' 카드 · 누르면 전환'} onClick={()=>{if(!lock.current){setDecisionMode(CARDS[entry.kind].type==='skill'?'prepare':'swing');setSelected(id);setSwingStack([]);setStackEdit(null);}}}/>)}</div>{isV10&&decisionMode==='swing'&&choice&&<BattleReadout choice={choice} kind={selected==='basic'?'basic':selectedEntry?.kind||'basic'} cardCount={1+activeStack.length} coach={b.turn<=2}/>}
{isV10&&decisionMode==='swing'&&selectedEntry&&selectedEntry.kind!=='bunt'&&b.growthMode!=='patience'&&<section className="swing-stack v11-stack-system" aria-label="스윙 스택"><div className="stack-head"><div><span>9ZONE STACK · PLACE → CONNECT → SWING</span><strong>같은 카드도 순서가 스윙을 바꾼다.</strong><p>메인에서 시작해 ②→③→④ 순서로 존을 잇습니다. 이어진 경로는 카드 수로 잃은 HP 피해 효율을 일부 회복합니다.</p></div><div className="stack-efficiency"><small>최종 HP 피해</small><b>{Math.round((choice?.damageRate??1)*100)}%</b><em>{1+activeStack.length}장 · CONNECT {choice?.stackPlan?.connectCount||0}/{Math.max(0,activeStack.length)}</em></div></div><StackBoard plan={choice?.stackPlan} activeId={stackEdit} onSelect={id=>{if(activeStack.some(x=>x.id===id))setStackEdit(id)}} onMove={moveStackOrder} onAim={(id,z)=>setSwingStack(xs=>xs.map(x=>x.id===id?{...x,aimZone:z}:x))} damageRate={choice?.damageRate??1} baseDamageRate={choice?.baseStackDamageRate??1} connectBonus={choice?.connectBonus||0} precisionPressure={choice?.precisionPressure||0}/><div className="stack-tradeoff" aria-label="카드 수별 HP 피해 효율">{V10_SWING_DAMAGE_RATES.map((rate,i)=><div key={rate} className={(1+activeStack.length===i+1?'current ':'')+(i<activeStack.length+1?'filled':'')}><span>{i+1}장</span><b>{Math.round(rate*100)}%</b><small>{['강공','균형','안전','올인'][i]}</small></div>)}</div><div className="stack-lane" aria-label="현재 겹친 카드"><div className="stack-slot main"><span>MAIN</span><strong>{CARDS[selectedEntry.kind].name}{selectedEntry.plus?'+':''}</strong><small>{ZONES[b.aimZone]} · 원래 효과 유지</small></div>{Array.from({length:V10_SWING_STACK_MAX-1},(_,i)=>{const x=activeStack[i],entry=x?byId(x.id):null;return x?<button type="button" key={x.id} className={'stack-slot support '+(stackEdit===x.id?'editing':'')} onClick={()=>setStackEdit(x.id)}><span>#{i+2}</span><strong>{CARDS[entry?.kind]?.name}{entry?.plus?'+':''}</strong><small>{ZONES[x.aimZone]} · {choice?.stackPlan?.links?.[i]?.connected?'CONNECT +'+Math.round(V11_STACK_CONNECT_BONUS*100)+'%':'BREAK · 커버'}</small><i aria-hidden="true">편집</i></button>:<div key={'empty'+i} className="stack-slot empty"><span>+{i+1}</span><strong>카드 추가</strong><small>{i===0?'여기서 빈 코스를 막기':'선택 사항'}</small></div>})}</div><div className="stack-candidates" aria-label="겹칠 카드 선택">{supportCandidates.map(({id,entry})=>{const picked=activeStack.some(x=>x.id===id),full=activeStack.length>=V10_SWING_STACK_MAX-1&&!picked;return <button type="button" key={id} className={picked?'picked':''} aria-pressed={picked} disabled={full} onClick={()=>{if(picked){setSwingStack(xs=>xs.filter(x=>x.id!==id));if(stackEdit===id)setStackEdit(null);}else{setSwingStack(xs=>[...xs,{id,aimZone:b.aimZone}].slice(0,V10_SWING_STACK_MAX-1));setStackEdit(id);}}}><span>{picked?'✓ STACK':'+ 겹치기'}</span><strong>{CARDS[entry.kind].name}{entry.plus?'+':''}</strong><small>{AXIS_NAMES[CARDS[entry.kind].axis]||'커버'} · 효과는 미발동</small></button>})}{!supportCandidates.length&&<p>지금 손패에는 더 겹칠 스윙 카드가 없습니다.</p>}</div>{stackEditItem&&<div className="stack-aim-editor"><div className="stack-aim-title"><div><span>추가 카드 코스</span><strong>{CARDS[byId(stackEditItem.id)?.kind]?.name}은 어디를 막을까?</strong></div><button type="button" onClick={()=>{setSwingStack(xs=>xs.filter(x=>x.id!==stackEditItem.id));setStackEdit(null)}}>이 카드 빼기</button></div><div className="assist-zone-grid" role="group" aria-label="추가 카드 노릴 존">{ZONES.map((name,z)=><button type="button" key={z} aria-label={'추가 카드 '+name} aria-pressed={stackEditItem.aimZone===z} className={stackEditItem.aimZone===z?'selected':''} onClick={()=>setSwingStack(xs=>xs.map(x=>x.id===stackEditItem.id?{...x,aimZone:z}:x))}><span>{z+1}</span><small>{name}</small></button>)}</div></div>}<div className="stack-verdict"><div><span>현재 커버</span><b>{choice?.coverage?.length||0}<i>/9</i></b></div><div><span>카드 소모</span><b>{1+activeStack.length}<i>장</i></b></div><div className="connect"><span>CONNECT</span><b>{choice?.stackPlan?.connectCount||0}<i>/{Math.max(0,activeStack.length)}</i></b></div><div className="damage"><span>HP 피해</span><b>{Math.round((choice?.damageRate??1)*100)}<i>%</i></b><small>{activeStack.length?Math.round((choice?.baseStackDamageRate??1)*100)+'% + 연결 '+Math.round((choice?.connectBonus||0)*100)+'%':'FULL POWER'}</small></div><p>{activeStack.length?'카드를 많이 쓰면 기본 피해 효율은 내려갑니다. 대신 ①→②→③→④의 노림존을 이어 CONNECT를 만들면 링크마다 '+Math.round(V11_STACK_CONNECT_BONUS*100)+'%p를 되찾습니다. 같은 카드라도 순서를 바꿔 경로를 다시 설계할 수 있습니다.':'한 장 승부는 HP 피해 100%. 카드를 더 놓으면 커버와 순서 설계가 시작됩니다.'}</p></div></section>}<div className={'decision-preview '+(choice?'active':'')}>{choice?<><div><b>{choice.problem||choice.label}</b><span>{selectedEntry&&CARDS[selectedEntry.kind].type==='skill'?'준비 1회 · 공 소비 없음':'공 1개 진행 · 스윙은 한 번'}</span>{choice.hit!=null&&<><span>{choice.coverageLabel}{choice.sacrifice>0?' · 희생 작전':' · 범위 적중 시 안타'}</span><span>{choice.sacrifice>0?'희생 작전':'적중 뒤 실패 판정 없음'}{choice.sacrifice>0?' · 스트라이크일 때 '+(selectedEntry?.plus?'85':'70')+'% 성공':''} · 코스 확률과 타격 범위로 계산합니다.</span></>}{readLevel(s)===2&&choice.types&&choice.hit>0&&<span className="hit-profile">적중 후 안타 종류: {choice.types.filter(t=>t.p>.005).map(t=>t.label+' '+pct(t.p)).join(' / ')}</span>}{choice.growthText&&<b className="growth-preview">{choice.growthText}</b>}{choice.events?.slice(0,2).map((e,i)=><span key={i}>{e}</span>)}</div>{selectedDetailName&&<button type="button" className="card-detail-open" aria-haspopup="dialog" aria-label={selectedDetailName+' 카드 설명 열기'} onClick={e=>{detailOpener.current=e.currentTarget;setCardDetail(cardDetailOf(selectedEntry?selectedEntry.kind:'basic',!!selectedEntry?.plus,choice.problem));}}>ⓘ 설명</button>}{isV10&&decisionMode==='swing'&&activeStack.length>0&&choice?.stackPlan&&<button type="button" className="card-order-open" aria-haspopup="dialog" aria-label={'스윙 순서 편집 · '+(1+activeStack.length)+'장'} onClick={e=>{orderOpener.current=e.currentTarget;setOrderOpen(true);}}>순서 {1+activeStack.length}</button>}<button className="primary" disabled={!!choice.problem||!!fx} onClick={()=>act(doPlay,true,choice?.stackPlan)} data-testid="execute-action">{isV10&&decisionMode==='swing'&&selectedEntry?(activeStack.length?(1+activeStack.length)+'장 배치 스윙 · 피해 효율 '+Math.round((choice?.damageRate??1)*100)+'%':CARDS[selectedEntry.kind].name+(selectedEntry.plus?'+':'')+' · 단독 스윙 · 피해 효율 100%'):selected==='basic'?'기본 스윙':CARDS[selectedEntry?.kind]?.name+(selectedEntry?.plus?'+':'')+(CARDS[selectedEntry?.kind]?.type==='skill'?' · 준비 1회':' · 스윙')}</button></>:decisionMode==='prepare'?<><b>준비 카드를 고르세요.</b><span>준비는 공을 쓰지 않습니다 · 남은 준비 {prepLeft}회 · 고른 뒤 실행하면 스윙 전에 적용됩니다.</span></>:<><b>카드를 9존에 놓으세요.</b><span>카드를 놓은 위치가 노림존이 됩니다. 추가 카드를 더 놓으면 커버가 넓어집니다.</span></>}</div></section>}</>}<div className="table-footer"><div className="pile-buttons"><button onClick={()=>setModal('draw')}>뽑기 {b.draw.length}</button><button onClick={()=>setModal('discard')}>버림 {b.discard.length}</button></div><span className="deck-rule">선수는 베이스에 · 카드는 행동에</span></div></section></main>
    :isV10&&s.phase==='map'?<BallparkMap key="screen-bp-map" {...selectV10Map(s)} completedIds={s.runMap.completedNodeIds} deckCount={s.deck.length} relicCount={s.relics.length}
      portraits={pitcherPortraits} onInspect={openPitcher} onEnter={id=>{const why=v10NodeProblem(current.current,id);if(why){setError(why);return}setError('');act(x=>enterV10Node(x,id))}}/>
    ...
    :isV10?<BallparkEnd key="screen-bp-end" won={s.phase==='won'} pitcher={s.pitcher} portrait={pitcherPortraits[s.v10?.opponent?.artId]||null}
