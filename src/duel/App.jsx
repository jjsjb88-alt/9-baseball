...
const V4_SHEETS=[batterSwingV4,batterMissV4,batterHomerV4,pitcherPitchV4,pitcherStrikeoutV4];
const ACTOR_ASSETS=[...new Set([...BATTER_SWING_V2,...BATTER_MISS_V2,...PITCHER_PITCH_V2,...PITCHER_K_V2,...V4_SHEETS,batterHomerHeroV3,...Object.values(PITCHER_RELEASE_V3)])];
...
const V4_STAGE_RANGES={windup:[0,17],impact:[18,23],slowmo:[24,31],release:[32,52],settle:[53,59]};
const V4_K_RANGES={release:[0,39],settle:[40,59]};
function v4SpriteSpec(who,stage,shot){
  if(!stage||!shot||['read','lock','expand','signal','survive','draw'].includes(shot.grade))return null;
  const range=(who==='pitcher'&&(shot.grade==='strikeout'||shot.grade?.endsWith('-k'))&&V4_K_RANGES[stage])||V4_STAGE_RANGES[stage];
  if(!range)return null;
  if(who==='pitcher'){
    const strikeout=(shot.grade==='strikeout'||shot.grade?.endsWith('-k'))&&['release','settle'].includes(stage);
    return {sheet:strikeout?pitcherStrikeoutV4:pitcherPitchV4,start:range[0],end:range[1],duration:Math.max(1,stageDuration(stage,shot)||180)};
  }
  const miss=['near-miss','near-miss-k','chase','chase-k','fooled','strikeout'].includes(shot.grade);
  const homer=['homer','grand-slam'].includes(shot.grade);
  return {sheet:homer?batterHomerV4:miss?batterMissV4:batterSwingV4,start:range[0],end:range[1],duration:Math.max(1,stageDuration(stage,shot)||180)};
}
function useV4Frame(spec,key){
  const [index,setIndex]=useState(spec?.start||0);
  useEffect(()=>{
    if(!spec){setIndex(0);return;}
    const {start,end,duration}=spec;setIndex(start);
    if(end<=start)return;
    let raf=0,started=null,last=start;
    const tick=now=>{
      if(started===null)started=now;
      const p=Math.min(1,(now-started)/Math.max(1,duration));
      const next=Math.min(end,start+Math.floor(p*(end-start+1)));
      if(next!==last){last=next;setIndex(next);}
      if(p<1)raf=requestAnimationFrame(tick);
    };
    raf=requestAnimationFrame(tick);
    return ()=>cancelAnimationFrame(raf);
  },[spec?.sheet,spec?.start,spec?.end,spec?.duration,key]);
  return index;
}
function V4Frame({sheet,index}){
  const col=index%10,row=Math.floor(index/10);
  const x=col===0?0:col/9*100,y=row===0?0:row/5*100;
  return <i aria-hidden="true" className="duel-sprite v4-sheet-frame" style={{backgroundImage:`url(${sheet})`,backgroundPosition:`${x}% ${y}%`}}/>;
}
function Sprite({who,stage=null,shot=null,golden=false,variant=null}){
  const pose=actorPose(who,stage,shot),v4=golden?v4SpriteSpec(who,stage,shot):null,v4Index=useV4Frame(v4,who+'-'+stage+'-'+(shot?.grade||'idle')+'-'+(variant||'base'));
  const spec=v4?null:sequenceSpec(who,stage,shot),animated=useSpriteFrame(spec,who+'-'+stage+'-'+(shot?.grade||'idle')+'-'+(variant||'base'));
  const legacy=(who==='pitcher'?PITCHER_POSES:BATTER_POSES)[pose],v2=golden?V2_FALLBACKS[who]?.[pose]?.():null,authored=!v4&&golden?authoredActorArt(who,pose,stage,shot,variant):null,src=authored||animated||v2||legacy;
  return <span className={'sprite-stage sprite-'+who+' pose-'+pose+(v4?' v4-sequence':'')+(animated&&!authored?' v2-sequence':'')+(authored?' v3-authored':'')+(golden?' golden-actor':'')+(variant?' variant-'+variant:'')}>
    {golden&&<i className="actor-contact-shadow" aria-hidden="true"/>}
    {v4?<V4Frame sheet={v4.sheet} index={v4Index}/>:<><img aria-hidden="true" className="sprite-echo echo-back" src={src}/><img aria-hidden="true" className="sprite-echo echo-mid" src={src}/><img aria-hidden="true" className="duel-sprite" src={src}/></>}
...
function CinemaLab({sound,onBack}){
  ...
  return <main className={'cinema-lab release-combat stadium-stage-3 rival-game'+(shot?' fx-'+shot.kind:'')+(shot?.motion?.shake?' shake-'+shot.motion.shake:'')+(stage?' fx-stage-'+stage:'')}>
    ...
    <section className={'cinema-lab-stage duel-arena stadium golden-master-stage '+(renderer2?'renderer2-host ':'renderer1-host ')+arenaPresentationClass(stage,shot)} aria-label="연출 검수 무대">
      ...
      <div className="actor-left"><Sprite who="batter" stage={stage} shot={shot} golden/><span className="batter-nameplate">#09 TEST BATTER</span></div><div className="actor-right"><Sprite who="pitcher" stage={stage} shot={shot} golden/><span className="pitcher-nameplate">TEST PITCHER</span></div>
