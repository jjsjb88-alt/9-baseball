...
export default function Duel(){
  ...
  const b=s?.battle,showBattle=screen==='run'&&b&&(['battle','pitch','between'].includes(s.phase)||fx),fxPresentation=fx?presentationFor(s):null,byId=id=>s.deck.find(c=>c.id===id),choice=selected&&(selected==='basic'||b?.hand.includes(selected))?showPreview(selected):null;
  ...
  function act(fn,animate=false){if(lock.current||tour.open)return;const next=fn(current.current);if(next===current.current)return;persist(next);setSelected(null);setGrowthChoice(null);setRewardAction(null);setFacilityChoice(null);setDecisionMode(null);if(animate){const shot=presentationFor(next);lock.current=true;if(sound)cue(shot?.cue||next.last.kind);setFx(next.last);setFrame(1);const reduced=window.matchMedia?.('(prefers-reduced-motion: reduce)').matches,dramatic=['homer','extra','whiff','called','out'].includes(shot?.kind);timers.current=[setTimeout(()=>setFrame(2),reduced?10:dramatic?210:160),setTimeout(()=>{setFx(null);setFrame(0);lock.current=false;timers.current=[]},reduced?60:dramatic?1120:880)];}}
