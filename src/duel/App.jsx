...
import {createV10Duel,enterV10Node,playV10Action,advanceV10Pitch,advanceV10Batter,claimV10Reward,
  ...
  saveV10Duel,readV10Duel} from './engine.js';
...
function JudgementVisual({shot,stage}){
  if(!shot)return null;
...
export default function Duel(){
  ...
  const [s,setS]=useState(initial.save),[screen,setScreen]=useState(()=>typeof window!=='undefined'&&new URLSearchParams(window.location.search).get('cinema')==='1'?'cinema':'menu'),[modal,setModal]=useState(null),[selected,setSelected]=useState(null),[decisionMode,setDecisionMode]=useState(null),[tour,setTour]=useState({open:false,step:0}),[tourRect,setTourRect]=useState(null),[fx,setFx]=useState(null),[fxStage,setFxStage]=useState(null),[frame,setFrame]=useState(0),[error,setError]=useState(initial.error||''),[sound,setSound]=useState(false);
  ...
  const b=s?.battle,showBattle=screen==='run'&&b&&(['battle','pitch','between'].includes(s.phase)||fx),fxPresentation=fx?presentationFor(s):null,fxStakes=fx?stakesFor(s):null,resultPresentation=b?.revealed?presentationFor(s):null,byId=id=>s.deck.find(c=>c.id===id),choice=selected&&(selected==='basic'||b?.hand.includes(selected))?showPreview(selected):null;
  ...
  const isV10=s?.version===10;
  /* V10은 V9 전투 화면을 그대로 쓴다. 바뀌는 건 승리 조건(투수 HP)과 행동이 지나는 진입점뿐이다. */
  const doPlay=x=>isV10?playV10Action(x,{type:'card',id:selected}):playCard(x,selected);
  ...
  function freshV10(){setGrowthChoice(null);setRewardAction(null);setFacilityChoice(null);persist(createV10Duel(Number(trialSeed)>>>0));setScreen('run');setModal(null);setSelected(null);setDecisionMode(null);setTour({open:false,step:0});}
