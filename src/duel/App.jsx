...
import {createV10Duel,enterV10Node,playV10Action,advanceV10Pitch,advanceV10Batter,claimV10Reward,
  ...
  saveV10Duel,readV10Duel,previewV10Combo} from './engine.js';
...
function CinemaLab({sound,onBack}){
  ...
  return <main className={'cinema-lab release-combat stadium-stage-3 rival-game'+(shot?' fx-'+shot.kind:'')+(shot?.motion?.shake?' shake-'+shot.motion.shake:'')+(stage?' fx-stage-'+stage:'')}>
    ...
    <section className={'cinema-lab-stage duel-arena stadium '+(renderer2?'renderer2-host ':'renderer1-host ')+arenaPresentationClass(stage,shot)} aria-label="연출 검수 무대">
...
export default function Duel(){
  ...
  const [s,setS]=useState(initial.save),[screen,setScreen]=useState(()=>typeof window!=='undefined'&&new URLSearchParams(window.location.search).get('cinema')==='1'?'cinema':'menu'),[modal,setModal]=useState(null),[selected,setSelected]=useState(null),[assist,setAssist]=useState(null),[assistZone,setAssistZone]=useState(4),[decisionMode,setDecisionMode]=useState(null),[tour,setTour]=useState({open:false,step:0}),[tourRect,setTourRect]=useState(null),[fx,setFx]=useState(null),[fxStage,setFxStage]=useState(null),[frame,setFrame]=useState(0),[error,setError]=useState(initial.error||''),[sound,setSound]=useState(false);
  ...
  const assistActive=!!(isV10&&assist&&selected&&b?.hand.includes(assist)&&selectedEntry?.kind!=='bunt'&&b.growthMode!=='patience');
  const rawChoice=selected&&(selected==='basic'||b?.hand.includes(selected))?(assistActive?previewV10Combo(s,selected,assist,assistZone):showPreview(selected)):null;
  const choice=rawChoice&&assistActive&&rawChoice.coverage?{...rawChoice,coverageLabel:rawChoice.coverage.length+'존 합동 커버'}:rawChoice;
  ...
  const doPlay=x=>isV10?playV10Action(x,{type:'card',id:selected,...(assistActive?{assistId:assist,assistZone}:{})}):playCard(x,selected);
  ...
  useEffect(()=>()=>timers.current.forEach(clearTimeout),[]);
  useEffect(()=>{const close=e=>{if(e.key==='Escape'){setModal(null);setDecisionMode(null);setSelected(null);setAssist(null);tourDismissed.current=true;try{localStorage.setItem(TOUR_KEY,'done')}catch{}setTour(t=>t.open?{open:false,step:0}:t);}};window.addEventListener('keydown',close);return()=>window.removeEventListener('keydown',close)},[]);
  ...
  function openDecision(mode){
    ...
    setSelected(null);setAssist(null);setDecisionMode(mode);
