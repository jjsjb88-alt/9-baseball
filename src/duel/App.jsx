...
// V10 is a separate loop on the same deck rules. V9 stays the default so every
// existing entry point, save and test keeps landing where it always did.
export default function Duel(){
  const [mode,setMode]=useState(()=>typeof window!=='undefined'&&window.location.hash==='#v10'?'v10':'v9');
  const swap=next=>{setMode(next);if(typeof window!=='undefined')window.location.hash=next==='v10'?'v10':'';};
  return mode==='v10'?<V10App onExit={()=>swap('v9')}/>:<DuelV9 onV10={()=>swap('v10')}/>;
}
