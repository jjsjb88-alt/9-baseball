...
function CinematicStadiumDepth({stage=null,shot=null}){
  const power=['dead-center','extra','homer','grand-slam'].includes(shot?.grade),danger=['near-miss','near-miss-k','chase','chase-k','fooled','strikeout'].includes(shot?.grade);
  return <div className={'cinematic-stadium-depth depth-'+(stage||'idle')+(power?' depth-power':'')+(danger?' depth-danger':'')} aria-hidden="true">
    <div className="depth-back">
      <span className="depth-skyline"/>
      <span className="depth-score-tower"><i/><i/><i/><i/></span>
      <span className="depth-light-tower tower-left"><i/><i/><i/><i/></span>
      <span className="depth-light-tower tower-right"><i/><i/><i/><i/></span>
      <span className="depth-haze haze-back"/>
    </div>
    <div className="depth-mid">
      <span className="depth-crowd crowd-left"/>
      <span className="depth-crowd crowd-right"/>
      <span className="depth-dugout dugout-left"/>
      <span className="depth-dugout dugout-right"/>
      <span className="depth-haze haze-mid"/>
      <span className="depth-keylight key-batter"/>
      <span className="depth-keylight key-pitcher"/>
      <span className="depth-contact-light"/>
    </div>
    <div className="depth-front">
      <span className="depth-fence"/>
      <span className="depth-rail rail-left"/>
      <span className="depth-rail rail-right"/>
      <span className="depth-grass-blade grass-a"/>
      <span className="depth-grass-blade grass-b"/>
      <span className="depth-dust-front"/>
    </div>
  </div>;
}
...
function CinemaLab({sound,onBack}){
  ...
  return <main className={'cinema-lab release-combat stadium-stage-3 rival-game'+(shot?' fx-'+shot.kind:'')+(shot?.motion?.shake?' shake-'+shot.motion.shake:'')+(stage?' fx-stage-'+stage:'')}>
    ...
    <section className={'cinema-lab-stage duel-arena stadium '+(renderer2?'renderer2-host ':'renderer1-host ')+arenaPresentationClass(stage,shot)} aria-label="연출 검수 무대">
      ...
      <CinematicStadiumDepth stage={stage} shot={shot}/><PixelCinema stage={stage} shot={shot}/><PixelVFX stage={stage} shot={shot} token={token} drawCore={!renderer2}/><ReadTrace s={chosen.state} stage={stage} shot={shot}/>
