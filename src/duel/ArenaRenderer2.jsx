...
'vec2 camera(vec2 uv){float zoom=1.,pan=0.,tilt=0.;if(u_stage==1){float handoff=smoothstep(.18,.92,u_phase);zoom=mix(1.035,1.075,handoff);pan=mix(.038,-.020,handoff);tilt=mix(.006,-.004,handoff);}if(u_stage==2){zoom=1.118;pan=u_danger>.5?.004:-.030;tilt=-.010;}if(u_stage==3){zoom=1.138;pan=u_danger>.5?.010:-.038;tilt=-.012;}if(u_stage==4){zoom=1.115-.072*u_phase;pan=mix(u_danger>.5?.010:-.034,u_power>.7?-.048:.008,u_phase);tilt=mix(-.010,u_power>.7?-.020:.006,u_phase);}if(u_stage==5){zoom=1.040-.030*u_phase;pan=.006*(1.-u_phase);}vec2 p=uv-.5;p/=zoom;p+=vec2(pan,tilt);return p+.5;}',
...
' float L=lightAt(uv,vec2(.12,.02),.23)+lightAt(uv,vec2(.88,.02),.23);float beam=max(0.,1.-abs((uv.x-.12)-uv.y*.19)*8.)+max(0.,1.-abs((.88-uv.x)-uv.y*.19)*8.);float intensity=.11+.07*u_match+.08*u_rival;col+=vec3(.83,.76,.54)*L*intensity+vec3(.54,.67,.66)*beam*.025;float releasePulse=u_stage==1?smoothstep(.42,.82,u_phase)*(1.-smoothstep(.86,1.,u_phase)):0.;float releaseLight=lightAt(uv,vec2(.72,.47),.105);col+=vec3(.42,.82,.82)*releaseLight*releasePulse*.34;float tunnel=lineMask(uv,vec2(.70,.48),vec2(.40,.60),.006);col+=vec3(.52,.78,.72)*tunnel*releasePulse*.12;',
