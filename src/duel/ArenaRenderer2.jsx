...
'vec2 camera(vec2 uv){float zoom=1.,pan=0.,tilt=0.;if(u_stage==1){zoom=1.+.032*u_phase;pan=-.012*u_phase;tilt=-.004*u_phase;}if(u_stage==2){zoom=1.075;pan=u_danger>.5?.014:-.010;tilt=-.006;}if(u_stage==3){zoom=1.095;pan=u_danger>.5?.018:-.014;tilt=u_danger>.5?.004:-.010;}if(u_stage==4){zoom=1.08-.060*u_phase;pan=mix(u_danger>.5?.018:-.012,u_power>.7?-.034:.012,u_phase);tilt=mix(-.008,u_power>.7?-.018:.008,u_phase);}if(u_stage==5){zoom=1.025-.025*u_phase;pan=mix(u_danger>.5?.010:-.006,0.,u_phase);}vec2 p=uv-.5;p/=zoom;p+=vec2(pan,tilt);return p+.5;}',
...
' float horizon=.34,haze=smoothstep(.10,.40,uv.y)*(1.-smoothstep(.40,.59,uv.y));float distanceFade=smoothstep(.16,.43,uv.y)*(1.-smoothstep(.43,.62,uv.y));col=mix3(col,vec3(.18,.23,.21),haze*.27);col=mix3(col,vec3(.13,.18,.18),distanceFade*.10);',
