...
'vec2 camera(vec2 uv){float zoom=1.,pan=0.,tilt=0.;if(u_stage==1){zoom=1.+.034*u_phase;pan=-.011*u_phase;tilt=.004*u_phase;}if(u_stage==2){zoom=1.082;pan=u_danger>.5?.010:-.012;tilt=-.004;}if(u_stage==3){zoom=1.105;pan=u_danger>.5?.016:-.016;tilt=-.006;}if(u_stage==4){zoom=1.092-.062*u_phase;pan=mix(u_danger>.5?.016:-.014,u_power>.7?-.032:.011,u_phase);tilt=mix(-.006,u_power>.7?-.018:.008,u_phase);}if(u_stage==5){zoom=1.035-.025*u_phase;pan=.004*(1.-u_phase);}vec2 p=uv-.5;p/=zoom;p+=vec2(pan,tilt);return p+.5;}',
...
' float bankL=rectMask(uv,vec2(.105,.095),vec2(.055,.022)),bankR=rectMask(uv,vec2(.895,.095),vec2(.055,.022));vec2 lampCell=floor(uv*vec2(120.,180.));float lamp=step(.52,hash21(lampCell));col+=vec3(.78,.72,.50)*lamp*(bankL+bankR)*(.12+.09*u_match+.08*float(u_stage>=2));',
