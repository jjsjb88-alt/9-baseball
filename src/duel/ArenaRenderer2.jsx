...
' vec2 contact=vec2(.365,.70);float cd=length((uv-contact)*vec2(1.,1.25));if(u_stage==2)col+=mix(vec3(.95,.58,.25),vec3(.55,.95,.68),u_success)*exp(-cd*cd/.004)*(.45+.55*u_power);if(u_stage==3){col*=.72;col+=mix(vec3(.52,.16,.14),vec3(.18,.52,.46),u_success)*exp(-cd*cd/.018)*.34;}if(u_stage==4&&u_power>.65)col+=vec3(.52,.31,.12)*exp(-cd*cd/.028)*(1.-u_phase)*.32;',
' tracePlane(col,uv);',
