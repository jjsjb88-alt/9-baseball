...
const out=process.env.QA_OUT||'qa/batter-motion-loop-v3';
const viewports=[
  {name:'390x844',width:390,height:844},
  {name:'844x390',width:844,height:390},
  {name:'1440x900',width:1440,height:900},
];
const required=['ready','load','trigger','swing-start','swing-mid','contact','follow-through-early','follow-through-late','finish','settle'];
const capturePoses=[...required];
const fullPagePoses=new Set(['ready','swing-start','swing-mid','contact','follow-through-late','finish']);
...
for(const viewport of viewports){
  ...
  await dynamic.locator('.batter-reboot-v3.reboot-pose-ready .batter-reboot-art').waitFor({state:'visible',timeout:10000});
  await dynamic.evaluate(()=>{
    const root=document.querySelector('.batter-reboot-v3');
  ...
  const traceCore=trace.slice(0,10);
  ...
  for(const pose of capturePoses){
    ...
    await page.locator('.batter-reboot-v3.reboot-pose-'+pose+' .batter-reboot-art').waitFor({state:'visible',timeout:10000});
    ...
    const metrics=await page.evaluate(()=>{
      ...
      return {
        ...
        pose:document.querySelector('.batter-reboot-v3')?.dataset?.batterPose||'',
        naturalWidth:img?.naturalWidth||0,
        naturalHeight:img?.naturalHeight||0,
      };
    });
  ...
  const failures=[];
  const firstTen=trace.filter((pose,index)=>index===0||pose!==trace[index-1]).slice(0,10);
  if(JSON.stringify(firstTen)!==JSON.stringify(required))failures.push('dynamic pose order mismatch: '+firstTen.join(' > '));
  const dwellByPose=Object.fromEntries(dwell.map(item=>[item.pose,item.ms]));
  for(const pose of ['swing-start','swing-mid','follow-through-early','follow-through-late']){
    if((dwellByPose[pose]??0)<28)failures.push(pose+' was not painted long enough: '+(dwellByPose[pose]??0)+'ms');
  }
