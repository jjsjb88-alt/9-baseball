...
import {batterMotionV2Timeline} from '../src/duel/batterMotionV2.js';
...
describe('9-zone strategic UI',()=>{
  ...
  it('runs the full cinema presentation inside the MAIN RUN pitcher-HP battle, not only in the lab',()=>{
    ...
    expect(arena.querySelector('.sprite-batter.batter-reboot-v2.reboot-pose-contact img.batter-reboot-art')).toBeTruthy();
    finish();
  });
  ...
  it('keeps pitcher 60-frame playback while the batter uses authored reboot key poses and overlays the tactical read trace',()=>{
    ...
    fireEvent.click(screen.getByRole('button',{name:'밀어치기',exact:true}));
    fireEvent.click(screen.getByTestId('execute-action'));
    expect(document.querySelector('.sprite-batter.batter-reboot-v2.reboot-pose-ready img.batter-reboot-art')).toBeTruthy();
    ...
    const motion=batterMotionV2Timeline(shot);
    const load=motion.find(x=>x.pose==='load'),swingStart=motion.find(x=>x.pose==='swing-start'),contact=motion.find(x=>x.pose==='contact');
    act(()=>vi.advanceTimersByTime(load.at+1));
    expect(document.querySelector('.sprite-batter.batter-reboot-v2.reboot-pose-load img.batter-reboot-art')).toBeTruthy();
    act(()=>vi.advanceTimersByTime(swingStart.at-load.at));
    expect(document.querySelector('.sprite-batter.batter-reboot-v2.reboot-pose-swing-start img.batter-reboot-art')).toBeTruthy();
    act(()=>vi.advanceTimersByTime(contact.at-swingStart.at));
    expect(document.querySelector('.sprite-batter.batter-reboot-v2.reboot-pose-contact img.batter-reboot-art')).toBeTruthy();
