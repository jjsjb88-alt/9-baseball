...
describe('growth reward and in-game controls',()=>{
  ...
  it('waited-strike growth narrows the shown coverage and exposes the cost before swinging',()=>{
    ...
    fireEvent.click(screen.getByTestId('execute-action'));finish();
  ...
  it('luck reservation survives reload and a hit shows the actual extra base advance',()=>{
    ...
    fireEvent.click(screen.getByRole('button',{name:'스윙하기',exact:true}));fireEvent.click(screen.getByRole('button',{name:'밀어치기',exact:true}));fireEvent.click(screen.getByTestId('execute-action'));finish();
  ...
  it('older local saves are not overwritten by a new v9 run',()=>{
    ...
    render(<Duel/>);fireEvent.click(screen.getByRole('button',{name:'튜토리얼 시작'}));
